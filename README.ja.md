...
| ファイル                       | 説明                                                                                 |
| ------------------------------ | ------------------------------------------------------------------------------------ |
| `EasyCursorSwap_x64-setup.exe` | NSIS インストーラー（ユーザーインストール、管理者権限不要、日本語 / 英語を選択可能） |
| `EasyCursorSwap_x64_ja-JP.msi` | MSI インストーラー（サイレント / GPO 配布向け）                                      |

> 一般ユーザーには NSIS インストーラーがおすすめです（インストール時に日本語 / 英語を選択可能）。
> MSI はサイレント / 管理配布向けで、セットアップ UI は日本語です。アプリ本体の表示言語は
> インストーラーとは独立しており、アプリ内の設定に従います。
...
詳細なモジュール / IPC ドキュメントは、本リポジトリ外のメンテナのナレッジベースで管理されています。
