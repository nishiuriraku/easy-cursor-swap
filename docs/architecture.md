...
| **テーマパッケージ** | `theme/` | 7 ファイル: `types` (DTO + 内部 helper) / `sanitize` (path traversal 対策) / `mod` (entry + `ThemeManager` 定義 + 公開 re-export) / `listing` (`list_themes` / metadata 読込 / 孤児復旧 / `set_metadata_source`) / `preview` (`load_role_previews` 系 / `render_paths_as_previews_*` / `build_preview_pngs` / `RolePreview` / `PREVIEW_ROLES`) / `apply` (`apply_theme` / `theme_active_in_registry`) / `package` (`.cursorpack` zip 入出力 / `import_cursorpack_*` / `export_cursorpack` / `inspect_cursorpack_*` / `delete_theme` / `duplicate_theme` / `repackage_theme` / `export_scheme_as_cursorpack`)。`impl ThemeManager` は各サブモジュールに分散。 |
