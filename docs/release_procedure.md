...
> **タグ発行後の手順** (Tauri Updater Ed25519 署名 / MSIX 自動署名は別 workflow
> `build-msix-artifacts.yml` で扱う) は [`authenticode_signing.md`](./authenticode_signing.md)
> と [`updater_signing.md`](./updater_signing.md) を参照。本 runbook はタグ push
> までのリポジトリ側作業に範囲を限定する。
...
- [`authenticode_signing.md`](./authenticode_signing.md) — Authenticode 署名 (Microsoft Store / MSIX 自動署名中心) の戦略と、方針変更 (2026-07-25) と旧 SignPath 経緯
