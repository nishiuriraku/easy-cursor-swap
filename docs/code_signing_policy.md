...
送信先は easy-cursor-swap の所有する Cloudflare Worker のみ。第三者には開示しません。
**無効化方法**: `%LOCALAPPDATA%\EasyCursorSwap\config.json` の `crash_reporting: false`
に変更、または GUI の「設定 → クラッシュレポート」トグルを OFF。
