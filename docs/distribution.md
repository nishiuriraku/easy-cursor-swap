...
出力先:
- `src-tauri/target/release/bundle/msi/CursorForge_*.msi`
- `src-tauri/target/release/bundle/nsis/CursorForge_*-setup.exe`
...
# 2. パッケージ生成
makeappx pack /d msix-payload /p CursorForge.msix /v

# 3. 署名 (テスト用 self-signed certificate)
signtool sign /a /v /fd SHA256 /f cert.pfx /p "<password>" CursorForge.msix
```
...
{
  "endpoints": [
    "https://github.com/cursorforge/cursor-forge/releases/latest/download/latest.json"
  ]
}
```
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://github.com/cursorforge/cursor-forge/releases/download/v1.0.1/CursorForge_1.0.1_x64-setup.nsis.zip"
    }
  }
}
```
