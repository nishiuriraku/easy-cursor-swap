...
- [ ] [signpath.org/apply](https://signpath.org/apply) から OSS Foundation 申請
- [ ] 審査承認 (1〜2 週間)
- [ ] SIGNPATH_API_TOKEN / SIGNPATH_ORGANIZATION_ID を GitHub Secrets に登録
- [ ] テストタグでパイプラインを 1 回緑にする (test-signing で)
- [ ] Foundation 承認後、`release-signing` policy を SignPath UI で作成
- [ ] GitHub Variable `SIGNPATH_SIGNING_POLICY_SLUG` を `release-signing` に変更
- [ ] 署名済み `.msi` / `.exe` で SmartScreen 動作確認
- [ ] README に SmartScreen 警告対応の説明を追記済み (既存)
