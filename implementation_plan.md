...
- [ ] Phase 9.1: **手動 e2e 検証**（5 シナリオ — ユーザー実機確認）

### 手動 e2e で検証するシナリオ
1. サンプルパックフォルダ (`sample-icon/`) 取り込み → 17 ロール filled
2. 既存 5 ロール埋め後にフォルダ取り込み（既存保護 ON）→ 既存温存 + 12 追加
3. 命名違反フォルダ → 未マッチ手動マッピング → 適用成功
4. `.cursorpack` 取り込み → メタデータ反映 + 解像度別保持エクスポートのラウンドトリップ
5. 100 ファイルのフォルダ取り込み中にキャンセル → UI 応答性保持
...
### 既知の限界
- `bulk_resolve_assets` のキャンセルは現状チェックポイント未実装（IPC は残るが、進行中の loop が中断しない）。フォロー TODO。
- `primarySize` (PNG/SVG 単一インポート時) は 256 固定の暫定値。将来 IHDR から取り出すように改善予定。
